...
bool isValidMacAddress(const String& mac) {
    if (mac.length() != 17) return false;  // XX:XX:XX:XX:XX:XX = 17 chars
    ...
    for (int i = 0; i < 17; i++) {
        if (i % 3 == 2) {
            ...
            if (!isxdigit(mac.charAt(i))) return false;
        }
    }
    return true;
}
...
void webSetup()
{
    ...
    ESPUI.addControl(
        ...
        "1. On your NOVA Mini (the receiver), look for a sticker with its MAC address<br>"
...
ControlColor getColorForName(const char* colorName) {
    if (!colorName) return ControlColor::Dark;
    
    String color = String(colorName);
    color.toLowerCase();
    
    if (color == "red") return ControlColor::Alizarin;     // Red
    if (color == "green") return ControlColor::Wetasphalt; // Changed to a different color for green
    if (color == "blue") return ControlColor::Peterriver;  // Blue
    if (color == "yellow") return ControlColor::Sunflower; // Yellow
...
void webLoop()
{
    ...
    if (millis() - oldTime > 500)  // Changed from 1000 to 500
    {
        ...
        if (simona) {  // Changed this to get singleton instance
            String progressStr;
